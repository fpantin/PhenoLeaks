...
df <- read.csv(file.path(dir_Exp, "Processed_data", "C2M43B_pot_transpiration.csv"))
...
# based on  series of inconsistent data (that will be identified as grey points in the next plots).
  ...
  #df$outlier[df$Time >= 2 + 45/60/24 & df$Time < 2 + Pho_Per/24 - 15/60/24] <- T
  
  
  ## Individual pots
  
  # Col-0
  df$outlier[df$idPot == "75" & df$Time > 1 + Pho_Per/24/4 & df$Time <= 2 + Pho_Per/24] <- T
  df$outlier[df$idPot == "75" & df$Time > 3 & df$Exact_Time_min <= 3*60*24 + 90] <- T
  df$outlier[df$idPot == "75" & df$Exact_Time_min >= 3*60*24 + 270 & df$Exact_Time_min <= 3*60*24 + 330] <- T
  df$outlier[df$idPot == "77" & df$Time > 3 + Pho_Per/24] <- T
  #df$outlier[df$idPot == "78" & df$Time > Pho_Per/24 & df$Time <= 2] <- T
  df$outlier[df$idPot == "78" & df$Time > Pho_Per/24 & df$Exact_Time_min <= 1*60*24 + 210] <- T
  df$outlier[df$idPot == "78" & df$Time > 1 + Pho_Per/24 & df$Time <= 2] <- T
  df$outlier[df$idPot == "79" & df$Time <= 0] <- T
  df$outlier[df$idPot == "79" & df$Time > 1 & df$Exact_Time_min <= 1*60*24 + 210] <- T
  
  # bam1
  df <- df[!df$idPot %in% c("108"), ]
  df$outlier[df$idPot == "112" & df$Time > 3 + Pho_Per/24/2] <- T
  
  # bam1 bam3
  df <- df[!df$idPot %in% c("123", "124"), ]
  
  # bam3
  df$outlier[df$idPot == "116" & df$Time > 1 + Pho_Per/24 & df$Time <= 2] <- T
  df$outlier[df$idPot == "120" & df$Time > 0 & df$Time < 240/60/24] <- T

  # pgm
  df$outlier[df$idPot == "89" & df$Time > 1 + (Pho_Per+1+1/60)/24 & df$Time < 2 + (Pho_Per+1/60)/24] <- T # 1 min added to avoid rounding issues
  df$outlier[df$idPot == "92" & df$Time > 1 + (Pho_Per+1+1/60)/24 & df$Time < 2 + (Pho_Per+1/60)/24] <- T # 1 min added to avoid rounding issues
...
# This is required because lines have been deleted for plants on which all data were discarded (pots 108, 123, 124).
...
df$E_estimate <- df$E_estimate * df$meanVPD
