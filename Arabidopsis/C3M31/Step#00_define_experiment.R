...
set_constants_C3M31 <- function ()
  ...
  Time_start_exp <- -(Sko_Per+Pho_Per/2)/24 - 1/60/24 # 1 min subtracted to avoid rounding issues (the 'real' value should be included in the interval Time_start_exp:Time_end_exp)
...
set_colors_C3M31 <- function ()
  ...
  t1 <- c(-1, 2+3/24, 2+5/24)
  t2 <- c(2+3/24, 2+5/24, 3.5)
  ColorsPeriod <- data.frame(idPeriod = c("Control 1", "High VPD", "Control 2"),
                             Time1 = t1,
                             Time2 = t2,
                             decimalDay1 = Time_ON0 + t1,
                             decimalDay2 = Time_ON0 + t2,
                             col = c("black", "darkorange1", "black"))
...
# Call the function
c(ColorsTrt, ColorsPeriod) := set_colors_C3M31()
...
irrig_file <- file.path(here::here(), spcs, idExp, "Processed_data", "C3M31_info_irrigation.csv")
if (file.exists(irrig_file))
  ...
  # --> All pots have 2 irrigation events
...
rectangles_environment_C3M31 <- function (col.per = ColorsPeriod, # the dataframe containing the features of the different periods
  ...
  t1.high.VPD <- col.per[2, paste(Time_var, 1, sep = "")]
  t2.high.VPD <- col.per[2, paste(Time_var, 2, sep = "")]
  ...
  if (irrig_show_mode != "none" & file.exists(irrig_file))
    ...
    text(mean(t_irrig), usr[3] + 0.025*(usr[4] - usr[3]), "Rehydration", col = "black", cex = cex.env)
    #require(igraph)
